function F = GlobalPlasmaSystemWithDimensionsAndK(x,Act,B,expNo)
...
x(9)/x(12)-TeV - (.5-TeV)/P];
%x(26) - 6e+7*M/(Na*density_MgO)*(1-RxnProb*((x(2)/nNorm)*(x(14)/vMax))/(RxnProb*((x(2)/nNorm)*(x(14)/vMax)) + (B1*(sqrt((6*x(9)))-sqrt(eod)))*((x(14)/vMax)*((x(6)/nNorm)+(x(7)/nNorm)+(x(8)/nNorm)))))*(RxnProb*((x(2)/nNorm)*(x(14)/vMax))+(A1*(sqrt((6*x(9)))-sqrt(eos)))*((x(14)/vMax)*(x(8)/nNorm)))];%k8
